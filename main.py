# これは「分析APIを含む」最新版 main.py の一部を、外部プロンプト対応に差し替えたものです。
# 既存の分析フロー（Whisper→要約採点→保存）や upload/list/signed-url/delete はそのまま動作します。
...
# --- Storage: アップロード/一覧/署名URL/削除（既存と同じ） ---
...
async def upload_file(staff: str, file: UploadFile = File(...)):
    try:
        ...
        supabase.storage.from_(SUPABASE_BUCKET).upload(
            path, content, file_options={"content-type": content_type, "x-upsert": "true"},
        )
...
def get_openai_transcript(file_bytes: bytes, filename: str) -> str:
    ...
    if client is None:
        raise HTTPException(500, "OPENAI_API_KEY 未設定 or openai ライブラリなし")
