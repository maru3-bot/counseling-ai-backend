...
// 環境変数でAPIの接続先を切り替え（デフォルトはローカルAPI）
...
function App() {
  ...
  const handleAnalyze = async (filename, force = false) => {
    ...
    try {
      const res = await axios.post(
        `${API_BASE}/analyze/${staff}/${filename}${force ? "?force=true" : ""}`
      );
      setAnalyses((prev) => ({ ...prev, [filename]: res.data }));
      ...
      setErr(filename, "分析に失敗しました。ファイルサイズ/音声品質/サーバログを確認してください。");
    } finally {
      clearLoad(filename);
    }
  };
  ...
  const handleDelete = async (filename) => {
    ...
    try {
      ...
      setVideoUrls((prev) => {
        const cp = { ...prev }; delete cp[filename]; return cp;
      });
      setAnalyses((prev) => {
        const cp = { ...prev }; delete cp[filename]; return cp;
      });
      ...
      setErr(filename, "削除に失敗しました。権限やパスを確認してください。");
    } finally {
      clearLoad(filename);
    }
  };
    ...
    return (
      <div className="analysis-card" style={{ marginTop: 12 }}>
        <div style={{ marginBottom: 8 }}>
          <strong>モデル:</strong> {model_name} ({model_mode})　
          <strong>作成時刻:</strong> {created_at}
        </div>
        {a.summary && (
          <>
            <h4>要約</h4>
        ...
        return (
          <div key={name} style={{ marginBottom: "24px" }}>
            ...
            <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button onClick={() => handleDelete(name)} disabled={!!isLoading} style={{ backgroundColor: "#b91c1c", color: "#fff" }}>
                削除
              </button>
            </div>
